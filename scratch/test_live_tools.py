...
try:
    ...
    if rates and "INR" in rates:
        record("Forex", "Open-ER-API", "Tier 1", "SUCCESS", f"1 USD = ₹{rates['INR']:.2f} ({provider})")
    else:
...
try:
    ...
    if rates and "INR" in rates:
        record("Forex", "FawazAhmed CDN", "Tier 2", "SUCCESS", f"1 USD = ₹{rates['INR']:.2f} ({provider})")
    else:
...
try:
    ...
    if rates and "INR" in rates:
        record("Forex", "Frankfurter ECB", "Tier 3", "SUCCESS", f"1 USD = ₹{rates['INR']:.2f} ({provider})")
    else:
...
if er_key:
    try:
        ...
        if rates and "INR" in rates:
            record("Forex", "ExchangeRate-API Auth", "Tier 4", "SUCCESS", f"1 USD = ₹{rates['INR']:.2f} ({provider})")
        else:
            record("Forex", "ExchangeRate-API Auth", "Tier 4", "FAILED", "Empty or missing INR rate")
    except Exception as e:
...
if "INR" in OFFLINE_BASELINE_RATES:
    record("Forex", "Offline Rates Table", "Tier 5 (Offline)", "SUCCESS", f"1 USD = ₹{OFFLINE_BASELINE_RATES['INR']:.2f} (Static Baseline)")
...
try:
    ...
    if w1_map:
        ...
        record("Weather", "Open-Meteo API", "Tier 1", "SUCCESS", f"16-day forecast available: Max {day0.temp_max_c}°C, {day0.condition}")
    else:
...
try:
    ...
    if w2_map:
        ...
        record("Weather", "wttr.in JSON API", "Tier 2", "SUCCESS", f"3-day forecast available: Max {day0.temp_max_c}°C, {day0.condition}")
    else:
...
if owm_key:
    try:
        ...
        if w3_map:
            ...
            record("Weather", "OpenWeatherMap 5-Day", "Tier 3", "SUCCESS", f"6-day forecast available: Max {day0.temp_max_c}°C, {day0.condition}")
        else:
...
try:
    w4 = get_weather_forecast(lat, lon, start_date=date_list[0], end_date=date_list[-1], use_fixture=True)
    if w4 and len(w4.daily_forecasts) > 0:
        record("Weather", "Offline Climate Baseline", "Tier 4 (Offline)", "SUCCESS", f"Fixture/Climate: {w4.summary}")
except Exception as e:
...
try:
    ...
    if s1 and len(s1.results) > 0:
        record("Web Search", "Tavily Search API", "Tier 1", "SUCCESS", f"Fetched {len(s1.results)} items: '{s1.results[0].title[:45]}...'")
    else:
...
try:
    ...
    if s2 and len(s2.results) > 0:
        record("Web Search", "DuckDuckGo Open API", "Tier 2", "SUCCESS", f"Fetched {len(s2.results)} items: '{s2.results[0].title[:45]}...'")
    else:
...
if fc_key:
    try:
        ...
        req = urllib.request.Request(url, data=payload, headers={"Authorization": f"Bearer {fc_key}", "Content-Type": "application/json"})
        with urllib.request.urlopen(req, timeout=8.0) as resp:
            data = json.loads(resp.read().decode("utf-8"))
            web_items = data.get("data", {}).get("web", [])
            if web_items:
                record("Web Search", "Firecrawl Search API", "Tier 3", "SUCCESS", f"Fetched {len(web_items)} web results: '{web_items[0].get('title')[:45]}...'")
            else:
                record("Web Search", "Firecrawl Search API", "Tier 3", "FAILED", "HTTP 200 but empty web results")
    except Exception as e:
...
try:
    ...
    if s4 and len(s4.results) > 0:
        record("Web Search", "Offline Search Fixture", "Tier 4 (Offline)", "SUCCESS", f"Fetched {len(s4.results)} mock items: '{s4.results[0].title[:45]}...'")
except Exception as e:
...
if rapid_key:
    try:
        ...
        req = urllib.request.Request(url, headers={"x-rapidapi-key": rapid_key, "x-rapidapi-host": "sky-scrapper.p.rapidapi.com"})
        with urllib.request.urlopen(req, timeout=10.0) as resp:
            data = json.loads(resp.read().decode("utf-8"))
            itins = data.get("data", {}).get("itineraries", [])
            if itins:
                first_p = itins[0].get("price", {}).get("formatted", "N/A")
                record("Transport", "Sky Scraper (RapidAPI Flights)", "Tier 1A", "SUCCESS", f"Fetched {len(itins)} live flight itineraries (Lowest price: {first_p})")
            else:
                record("Transport", "Sky Scraper (RapidAPI Flights)", "Tier 1A", "FAILED", "Empty itineraries array")
    except Exception as e:
        record("Transport", "Sky Scraper (RapidAPI Flights)", "Tier 1A", "FAILED", str(e))
else:
    record("Transport", "Sky Scraper (RapidAPI Flights)", "Tier 1A", "SKIPPED", "RAPIDAPI_KEY not set")

# Test Flights Sky
try:
    t2 = _search_flights_sky("DEL", "BOM", "2026-10-15")
    if t2 and len(t2.options) > 0:
        record("Transport", "Flights Sky (RapidAPI)", "Tier 1B", "SUCCESS", f"Fetched {len(t2.options)} flights: {t2.options[0].carrier} ₹{t2.options[0].price_inr}")
    else:
        record("Transport", "Flights Sky (RapidAPI)", "Tier 1B", "FAILED", "HTTP 404 / Invalid Endpoint (Unsubscribed sub-API)")
except Exception as e:
...
try:
    ...
    if t3 and len(t3.options) > 0:
        record("Transport", "Indian Railway IRCTC (RapidAPI)", "Tier 1C", "SUCCESS", f"Fetched {len(t3.options)} trains: {t3.options[0].carrier} ₹{t3.options[0].price_inr}")
    else:
        record("Transport", "Indian Railway IRCTC (RapidAPI)", "Tier 1C", "FAILED", "HTTP 404 / Endpoint structure changed (Unsubscribed sub-API)")
except Exception as e:
...
try:
    ...
    if t4 and len(t4.options) > 0:
        record("Transport", "transport.rest API (Euro Rail)", "Tier 1D", "SUCCESS", f"Fetched {len(t4.options)} journeys: {t4.options[0].carrier} ₹{t4.options[0].price_inr}")
    else:
        record("Transport", "transport.rest API (Euro Rail)", "Tier 1D", "FAILED", "Station ID resolution empty")
except Exception as e:
...
try:
    ...
    if t5 and len(t5.options) > 0:
        record("Transport", "Live Web Search Fallback", "Tier 2", "SUCCESS", f"Retrieved {len(t5.options)} live flight options via Tavily/DuckDuckGo web search")
    else:
        record("Transport", "Live Web Search Fallback", "Tier 2", "FAILED", "No web search options parsed")
except Exception as e:
...
try:
    ...
    if t6 and len(t6.options) > 0:
        record("Transport", "Distance Physics Engine", "Tier 3 (Offline)", "SUCCESS", f"Calculated {len(t6.options)} physics estimates (Air ₹{t6.options[0].price_inr:,.2f})")
except Exception as e:
